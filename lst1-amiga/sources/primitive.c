...
printf(" 32=%d ",leftint);
			leftfloat = ((double)leftint)/32768.;
		/* WMK - was ((double) ((leftint/10) % 1000)) / 1000.0; */
			goto return_float;
...
puts("131");
		...
		case 150:	/* class edit */
			...
			if (! writeable(leftp)) {
				gettemp(tempname);
	sprintf(strbuffer,/*"cp %s %s"*/ "copy %s to %s", leftp, tempname);
# ifndef NOSYSTEM
				system(strbuffer);
# endif
				leftp = tempname;
...
# endif
   fprintf(stderr,"Can't happen number %d: %s .. So long!!!!\n", n, s);
   exit(1);
}
...
int rand(x) { /* WMK - from blue book, p205 */
	static long a = 27181, c = 13849;
	seed = (a * (long)seed  + c) & 0x7fff;
	return(seed);
}
